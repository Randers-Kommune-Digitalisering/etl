...
def get_mac_addresses(db_client):
    ...
    try:
        result = db_client.execute_sql(sql_command)
        lan_mac_list = get_lan_mac_addresses(db_client)
        lan_mac_set = set((unit, mac) for unit, mac in lan_mac_list if mac)

        mac_dict = {}
        for unit_name, mac_value in result:
            if not (unit_name.startswith('DQ') or unit_name.startswith('AP')):
                if (unit_name, mac_value) in lan_mac_set:
                    continue
                mac_dict.setdefault(unit_name, []).append(mac_value)

        filtered_result = []
        for unit_name, mac_list in mac_dict.items():
            if mac_list:
