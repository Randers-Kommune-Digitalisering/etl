...
base_url = "https://api.jobindsats.dk/v2/data/y09a02/json"
jobindsats_client = APIClient(base_url, JOBINDSATS_API_KEY)
period_url = "https://api.jobindsats.dk/v2/tables/y09a02/json/"
jobindsats_period_client = APIClient(period_url, JOBINDSATS_API_KEY)


def get_jobindsats_ledighedsydelse():
    try:
        logger.info("Starting jobindsats Ledighedsydelse")
        ...
        df['Periode ledighedsydelse'] = df['Periode'].apply(convert_to_datetime)
        ...
        filename = "SA" + "JobindsatsY09A02" + ".csv"

        if post_data_to_custom_data_connector(filename, file):
            logger.info("Successfully updated JobindsatsY09A02")
            return True
        else:
            return False
