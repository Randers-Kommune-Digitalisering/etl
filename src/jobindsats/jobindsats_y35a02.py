...
base_url = "https://api.jobindsats.dk/v2/data/y35a02/json"
...
def get_jobindsats_sho(): ## sho = Selvforsørgelses- og hjemrejseydelse samt overgangsydelse
    try:
        ...
        df['Periode SHO'] = df['Periode'].apply(convert_to_datetime)
        ...
        filename = "SA" + "JobindsatsY35A02" + ".csv"

        if post_data_to_custom_data_connector(filename, file):
            logger.info("Successfully updated JobindsatsY35A02")
            return True
        else:
            return False
