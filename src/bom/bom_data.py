        ...
        module.exports = async ({page}) => {
            try {
                ...
                try {
                    ...
                    let currentMonth = new Date(2023, 4, 1);
                    let isFirstIteration = true;

                    const allData = [];

                    while (currentMonth.getMonth() < 8 && currentMonth.getFullYear() === 2023) {
                        ...
                        const tilDato = firstDayNextMonth.toLocaleDateString('da-DK', options).replace(/\./g, '-');
