...
from jobindsats.jobindsats_y36a02 import (
...
def mock_dependencies():
    with patch('jobindsats.jobindsats_y36a02.logger') as mock_logger, \
         patch('jobindsats.jobindsats_y36a02.jobindsats_client.make_request') as mock_client_request, \
         patch('jobindsats.jobindsats_y36a02.jobindsats_period_client.make_request') as mock_period_request, \
         patch('jobindsats.jobindsats_y36a02.post_data_to_custom_data_connector') as mock_post_data:
        yield {
...
def test_get_jobindsats_kontanthjælp(mock_dependencies):
    ...
    result = get_jobindsats_kontanthjælp()

    assert result
    mock_dependencies['mock_logger'].info.assert_any_call("Starting jobindsats Kontanthjælp")
    mock_dependencies['mock_logger'].info.assert_any_call("Successfully updated JobindsatsY36A02 ")
    mock_dependencies['mock_post_data'].assert_called_once()
    mock_dependencies['mock_client_request'].assert_called_once()


def test_get_jobindsats_kontanthjælp_post_data_failure(mock_dependencies):
    mock_dependencies['mock_period_request'].return_value = [{'Period': ['2024M01', '2024M02']}]
    mock_dependencies['mock_client_request'].return_value = [{
        'Variables': [{'Label': 'Periode'}, {'Label': 'Area'}],
        'Data': [['2024M01', 'Randers'], ['2024M02', 'Aarhus']]
...
def test_get_jobindsats_kontanthjælp_exception(mock_dependencies):
...
def test_period_request(mock_dependencies):
    mock_dependencies['mock_period_request'].return_value = [{'Period': ['2024M01', '2024M02', '2024M03', '2023M09', '2024M04', '2024M05']}]
    result = period_request()

    assert result == '2024M05'
